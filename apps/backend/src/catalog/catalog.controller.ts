...
export async function getProductAvailability(req: Request, res: Response): Promise<void> {
  const { sku } = skuParamSchema.parse(req.params);
...
export async function getCatalogSchema(_req: Request, res: Response): Promise<void> {
  res.status(200).json({
    description:
      'Product catalog conforming to schema.org Product, extended with agent-safety fields under `extensions`.',
    ...
    fields: {
      ...
      extensions: {
        ...
        maxAutoApproveAmount:
          'Amount (paise) below which a purchase may be treated as low-risk by the gating engine. Still always requires explicit user confirmation.',
        requiresConfirmationAbove:
          'Amount (paise) above which the gating engine must escalate before allowing purchase.',
