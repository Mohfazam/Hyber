...
export const searchCatalogDeclaration = {
    name: 'search_catalog',
    description: 'Search the product catalog by free-text query and/or filters (category, gender, size, price range). Use this whenever the user describes what they want to buy.',
    parameters: {
        type: Type.OBJECT,
        properties: {
...
export const getProductDetailsDeclaration = {
    ...
    description: 'Get full details for one specific product by SKU, including its voice-friendly description for responding to the user.',
    parameters: {
        type: Type.OBJECT,
        properties: {
...
export const proposePurchaseDeclaration = {
    name: 'propose_purchase',
    description: 'Propose completing a purchase for a specific product. ONLY call this after you have stated the exact product, quantity, and price out loud to the user AND the user has given an explicit, unambiguous "yes". Never call this on an implied or soft agreement � if in doubt, ask the user to confirm again first.',
    parameters: {
        type: Type.OBJECT,
        properties: {
...
async function executeGetProductDetails(args) {
 ...
 */
async function executeProposePurchase(args, sessionId) {
...
export async function executeTool(name, args, sessionId) {
    switch (name) {
        ...
        case 'propose_purchase':
            return executeProposePurchase(args, sessionId);
        default:
